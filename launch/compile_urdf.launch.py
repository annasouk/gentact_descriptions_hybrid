...
def build_robot_description(config):
    ...
    urdf_file = PathJoinSubstitution(
        [
            FindPackageShare("gentact_descriptions_hybrid"),
            config["robot"]["robot_xacro"],
        ]
    )
...
def launch_setup(context, *args, **kwargs):
    ...
    install_dir = os.path.dirname(
        os.path.dirname(os.path.dirname(package_share))
    )  # Remove share/gentact_ros_tools_hybrid
    ...
    output_file_resolved = os.path.join(
        source_package_dir, "urdf", "compiled", output_filename
    )
    ...
    mkdir_process = ExecuteProcess(
        cmd=mkdir_cmd, output="screen", name="mkdir_compile_dir"
    )

    xacro_process = ExecuteProcess(cmd=xacro_cmd, output="screen", name="xacro_compile")
...
def generate_launch_description():
    ...
    output_filename_arg = DeclareLaunchArgument(
        "output_filename",
        default_value="robot_model.urdf",
        description="Output URDF filename (compiled)",
    )
